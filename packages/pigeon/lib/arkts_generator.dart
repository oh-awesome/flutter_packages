 ...
 Generated enum Companion class from Pigeon that represents data sent 
   in messages.Do not delete otherwise enum type data transfer will be failed''',
    ...
    indent.addScoped('{', '}', () {
      ...
      indent.writeScoped('constructor(index: string) {', '}', () {
  ...
  void _writeClassField(
    ...
    final String getterReturnType =
        field.type.isNullable ? '$fieldType | undefined' : fieldType;
    ...
    indent.newln();
    indent.writeScoped(
      '${_makeGetter(field)}(): $getterReturnType {',
      '}',
      () {
  ...
  void writeFlutterApi(
    ...
    indent.addScoped('{', '}', () {
      ...
      indent.writeScoped('static getCodec(): MessageCodec<Object> {', '}', () {
  ...
  void _writeMethodSetup(
    ...
    indent.addScoped('{', '}', () {
      ...
      indent.addScoped('{', '} else {', () {
        ...
        indent.nest(2, () {
          indent.writeScoped('onMessage(message: Object, reply: Reply<Object>) {', '} });', () {
...
''');
  ...
  void _writeProxyApiRegistrar(
    ...
    for (final api in allProxyApis) {
      ...
      final bool hasHostHandlers = api.constructors.isNotEmpty ||
          ...
          indent.addScoped('{', '} });', () {
            ...
            if (isAsync) {
              final String resultType =
                  isVoid ? 'void' : _arkTSTypeOrEsObject(returnType);
              // Encode an enum result into its codec companion before replying
              // (matches the data-class toList enum handling).
