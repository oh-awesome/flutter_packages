...
class _VideoTracksDemoState extends State<VideoTracksDemo> {
  ...
  Future<void> _selectVideoTrack(VideoTrack? track) async {
    ...
    try {
      // MiniController.selectVideoTrack 接收 trackId（与 selectAudioTrack 对齐），
      // 这里将选中的 VideoTrack 转为 id 传入；track 为 null 表示自动质量。
      await controller.selectVideoTrack(track?.id);
          ...
          Card(
            ...
            child: ListTile(
              ...
              title: Text(
                ...
                style: TextStyle(
                  fontWeight:
                      _isAutoQuality ? FontWeight.bold : FontWeight.normal,
                ),
              ),
