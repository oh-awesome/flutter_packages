  ...
  testWidgets('Images from streaming have expected resolution from preset',
    ...
    for (final CameraDescription cameraDescription in cameras) {
      ...
      for (final MapEntry<ResolutionPreset, Size> preset
        ...
        final Completer<CameraImageData> imageCompleter =
            Completer<CameraImageData>();
        await controller.initialize();
        await controller.startImageStream((CameraImageData imageData) {
          imageCompleter.complete(imageData);
          controller.stopImageStream();
        });

        final CameraImageData imageData = await imageCompleter.future;
        final bool presetExactlySupported = assertExpectedDimensions(
            preset.value,
            Size(imageData.height.toDouble(), imageData.width.toDouble()));
        // Ensures that if a lower resolution was used for previous (lower)
        // resolution preset, then the current (higher) preset also is adjusted,
        // as it demands a higher resolution.
        expect(
