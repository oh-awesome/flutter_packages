...
void main() {
  ...
  group('Creation, Initialization & Disposal Tests', () {
    ...
    test('Should send creation data and receive back a camera id', () async {
      // Arrange
      final OhosCamera camera = OhosCamera(hostApi: mockCameraApi);
  ...
  group('Event Tests', () {
    ...
    setUp(() async {
      mockCameraApi = MockCameraApi();
      camera = OhosCamera(hostApi: mockCameraApi);
    ...
    test('Should receive resolution changes', () async {
      ...
      camera.cameraEventStreamController.add(fhdEvent);
      camera.cameraEventStreamController.add(uhdEvent);
      camera.cameraEventStreamController.add(fhdEvent);
      camera.cameraEventStreamController.add(uhdEvent);
  ...
  group('Function Tests', () {
    ...
    test('Should throw CameraException when setFocusMode fails', () {
      // Arrange
      when(mockCameraApi.setFocusMode(PlatformFocusMode.auto)).thenThrow(
        CameraException(
          'TESTING_ERROR_CODE',
          'Mock error message used during testing.',
        ),
      ...
      // Act
      expect(
        () => camera.setFocusMode(cameraId, FocusMode.auto),
        throwsA(
          isA<CameraException>()
              .having(
    ...
    test('Should set the focus point', () async {
      ...
      // Assert
      verify(mockCameraApi.setFocusPoint(argThat(predicate(
              (PlatformPoint point) => point.x == 0.5 && point.y == 0.5))))
          .called(1);
      verify(mockCameraApi.setFocusPoint(null)).called(1);
    });
    ...
    test('Should set the image file format', () async {
      ...
      // Assert
      verify(mockCameraApi.setImageFileFormat(PlatformImageFileFormat.jpeg))
          .called(1);
      verify(mockCameraApi.setImageFileFormat(PlatformImageFileFormat.heif))
          .called(1);
    });
  });
}
