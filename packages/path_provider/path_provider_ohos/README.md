...
本项目基于 [path_provider@2.1.4](https://pub.dev/packages/path_provider/versions/2.1.4) 开发。
...
| getExternalStoragePath()    |   Future<String?>         |        获取应用程序外部存储路径的方法，应用程序可以在其中访问外部存储的目录路径     |        function       | yes               |
| getExternalStoragePaths([StorageDirectory](#StorageDirectory) arg_directory)   | Future<List<String?>> |   获取应用程序外部存储根目录路径的方法，应用程序特定的数据可以存储外部目录的路径，这些路径通常位于外部存储上，如单独的分区或SD卡。手机可能有多个可用的存储目录 | function       | yes               |
