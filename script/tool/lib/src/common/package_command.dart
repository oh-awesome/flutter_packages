  ...
  PackageCommand(
    ...
    argParser.addFlag(
      ...
      help:
          ...
          'If no packages have changed, the command exits without running.\n'
          'If there have been changes that may affect all packages, the '
          'command runs on all packages.\n'
    ...
    if (changedFileFinder != null) {
      ...
      if (packages.isEmpty) {
        if (_changesRequireFullTest(changedFiles)) {
          print(
            'Files that could affect the entire repository have changed, '
            'but no specific packages were identified. '
        ...
        if (_changesRequireFullTest(changedFiles)) {
          print(
            'Files that could affect the entire repository have changed. '
            'Running only for changed packages: ${packages.toList()..sort()}',
          );
        } else {
          print(
      ...
      final GitVersionFinder gitVersionFinder = GitVersionFinder(
        await gitDir,
        baseSha: 'HEAD',
      );
